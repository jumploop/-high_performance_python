...
if __name__ == "__main__":
    ...
    if args.build:
        ...
        for _ in range(NBR_MORRIS_COUNTERS_TO_TRY):
            mc = morris_counter.MorrisCounter()
            values_in_morris_counter = []
            for _ in values_we_count:
                mc.add()
                values_in_morris_counter.append(mc.get())
            trials_of_morris_counting.append(values_in_morris_counter)
        pickle.dump((values_we_count,trials_of_morris_counting, values_in_morris_counter), open(PICKLE_FILENAME, 'wb'))
    else:
        print("Loading from", PICKLE_FILENAME, " writing to", PNG_FILENAME)
        (values_we_count,trials_of_morris_counting, values_in_morris_counter) = pickle.load(open(PICKLE_FILENAME, 'rb'))
        plt.figure(1)
        plt.clf()

        plt.plot(values_we_count, values_we_count, '-k', linewidth=3, label="Integer counter (Many bytes)")
        line_style = ['--', '-.', ':']
        for counter, values_in_morris_counter in enumerate(trials_of_morris_counting):
            plt.plot(
                ...
                line_style[counter],
                alpha=0.8,
                label=f"Morris counter {counter} (1 byte)",
            )
